...
class PlotService:
    ...
    def current_level(cls, level: float):
        ...
        def draw_pump_level(axs: Axes):
            line = Line2D([9, 11], [settings.tank.warning, settings.tank.warning], color='yellow')
            axs.add_line(line)
            plt.text(11.5, settings.tank.warning, 'критический \n уровень 1')

        def draw_critical_level(axs: Axes):
            line = Line2D([9, 11], [settings.tank.critical, settings.tank.critical], color='red')
            axs.add_line(line)
    ...
    def archive_levels(cls, data: list):
        ...
        plt.plot(x_vals, [settings.tank.warning] * len(x_vals), color='yellow', label='крит.ур.1')
        plt.plot(x_vals, [settings.tank.critical] * len(x_vals), color='red', label='крит.ур.2')
